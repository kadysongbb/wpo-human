...
def run_policy(env, policy, episodes, discriminator, lamb, logger):
    ...
    for e in range(episodes):
        observes, actions, rl_rewards = run_episode(env, policy)
        il_rewards = discriminator.get_reward(np.stack((observes, actions),1))
        rewards = rl_rewards*lamb + il_rewards*(1-lamb)
