...
def run_episode(env, policy, max_steps, animate=False):
    ...
    while not done and len(observes) <= max_steps:
        if animate:
            env.render() 
        observes.append(obs)
...
def run_policy(env, policy, episodes, max_steps, logger):
    ...
    for e in range(episodes):
        observes, actions, rewards = run_episode(env, policy, max_steps)
